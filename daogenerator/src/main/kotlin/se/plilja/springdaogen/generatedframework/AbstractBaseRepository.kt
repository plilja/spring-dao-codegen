...
fun abstractBaseRepository(_package: String): Pair<String, String> {
    return Pair(
...
public abstract class AbstractBaseRepository<T extends BaseEntity<ID>, ID> implements BaseRepository<T, ID> {
    ...
    protected AbstractBaseRepository(Class<ID> idClass, boolean idIsGenerated, NamedParameterJdbcTemplate jdbcTemplate) {
        ...
        if (object.getId() == null) {
            ...
            if (sql == null) {
                if (!existsById(object.getId())) {
                    ...
                    // Insert on a table with a natural id column that doesn't support updates (i.e. a one column table)
                    throw new UnsupportedOperationException("Updates are not supported on this table");
                }
            } else {
